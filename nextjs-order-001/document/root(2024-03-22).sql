...
-- 다음의 SQL 결과에서 데이터가 한개라도 나오면
-- FK 설정이 불가능하다
SELECT * FROM tbl_order_customer C
	LEFT JOIN tbl_orders O
		on C.oc_onum = O.o_num
WHERE O.o_num IS NULL;

SELECT * FROM tbl_order_product P
	LEFT JOIN tbl_orders O
...
-- 여기서 NULL 데이터가 있으면 안됨
SELECT * FROM tbl_order_product OP
	LEFT JOIN tbl_product P
		ON OP.op_pcode = P.p_code
WHERE P.p_code IS NULL;

SELECT * FROM tbl_order_customer OC
...
1. 한 개의 주문에는 반드시 고객은 한명 뿐이다
2. 한 개의 주문에는 여러개의 상품이 포함될수 있다.
단, 한 개의 주문에 같은 상품이 중복될수 없다.
*/
...
2. SELECT * FROM tbl_roders 명령을 자체적으로 실행한다
...
-- 대하여 시행할수 없도록 초기 세이팅 되어 있다.
-- 현재 tbl_orders 테이블 전체 데이터를 Update 를 실행해야
-- 하므로 잠시 safe update 를 해야한다
