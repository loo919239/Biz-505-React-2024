 ...
 * Pute(순수) React 에서는 모든것이 CSR 이기 때문에 별다른 조치 없이
 * 내장된 함수, 기능등을 사용할 수있다.
 * 하지만 NextJS 에서는 컴포넌트가 SSR 에서 동작하느냐, CSR 에서 동작하느냐에 따라
 * 사용할 수 있는 함수, 기능이 차이가 있다.
 * CSR 에서 동작하는 함수는 그냥 사용하면 사용불가 오류가 나타난다
 *
 * 이 컴포넌트에서는 useRouter 함수와, onClick 기능을 사용하려고 한다.
 * 이러한 경우에는 현재 컴포넌트가 CSR 환경에서 동작하도록 명시해주어야 한다.
...
export default () => {
  ...
  // db 등에서 fatch 한 데이터를 List 를 화면에 출력하는 방법
  const blogList = [
    { id: 1, subject: "안녕하세요" },
    { id: 2, subject: "반갑습니다" },
  ...
  const onClickHandler = (id) => {
    // alert(id);
    // router.push() 를 사용하면 함수에서 특별한 router 로 전환해 준다
    router.push(`/blog/detail/${id}`);
  };
  // event 핸들러에 어떤 값을 전달하고자 할때 핸들러 세팅
  // onClick={()=>호출할함수(전달할값)}
  // onClick={호출할함수(전달할값)} 처럼 사용하면 잘대 안된다
